...
const res: Response = await fetch('https://api.svgl.app?limit=2000');
const data = (await res.json()) as SvglEntry[];
const seen = new Set<string>();
let saved = 0;
let skipped = 0;

for (const tech of TECHNOLOGIES) {
  const filename: string = toFilename(tech);
  ...
  const searchTitle: string = SVGL_OVERRIDES[tech] ?? tech;
  const match: SvglEntry | undefined = data.find((s) => s.title.toLowerCase() === searchTitle.toLowerCase());
  if (!match) {
  ...
  const svgUrl: string =
    typeof match.route === 'string'
      ? match.route
      : ((match.route as { dark?: string; light?: string }).dark ??
        (match.route as { dark?: string; light?: string }).light ??
        '');
  try {
    const svgRes: Response = await fetch(svgUrl);
    const svgText: string = await svgRes.text();
    writeFileSync(join(ICONS_DIR, filename), svgText);
    console.log(`  ✔ ${tech} → ${filename}`);
    saved++;
  } catch (err) {
    console.log(`  ✘ ${tech} → download failed: ${(err as Error).message}`);
    skipped++;
  }
}
